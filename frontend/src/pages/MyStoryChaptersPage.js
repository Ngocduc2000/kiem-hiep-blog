...
import { getStory, getStoryChapters, getChapterForEdit, addChapterToStory, updateStoryChapter, deleteStoryChapter } from '../services/api';
...
export default function MyStoryChaptersPage() {
  ...
  useEffect(() => {
    ...
    getStory(storyId).then(res => setStory(res.data)).catch(() => navigate('/my-stories'));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = editing
        ? await updateStoryChapter(storyId, editing.id, form)
        : await addChapterToStory(storyId, form);
  ...
  const handleDelete = async (ch) => {
    ...
    try {
      await deleteStoryChapter(storyId, ch.id);
  ...
  if (!story) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>⏳ Đang tải...</div>;

  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <button className="btn btn-ghost btn-sm" onClick={() => navigate('/my-stories')}>← Quay lại</button>
